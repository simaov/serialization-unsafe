...
public class FloatSerializer extends Serializer {
    ...
    public byte[] serialize(Object baseObj, long fieldOffset) {
        return Utils.bytesFromInt(Float.floatToRawIntBits(UnsafeMemory.getPrimitiveFloat(baseObj, fieldOffset)));
    }

    @Override
    public int deserializeAndPutValue(byte[] bytes, int offset, Object instance, long fieldOffset) {
        UnsafeMemory.getUnsafe().putFloat(instance, fieldOffset, Float.intBitsToFloat(Utils.intFromBytes(bytes, offset)));
