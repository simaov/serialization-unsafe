...
public class DoubleObjectSerializer extends Serializer {
    @Override
    public byte[] serialize(Object baseObj, long fieldOffset) {
        return Utils.bytesFromLong(Double.doubleToRawLongBits(UnsafeMemory.getDoubleFieldValue(baseObj, fieldOffset)));
    }

    @Override
    public int deserializeAndPutValue(byte[] bytes, int offset, Object instance, long fieldOffset) {
        UnsafeMemory.getUnsafe().putObject(instance, fieldOffset, Double.longBitsToDouble(Utils.longFromBytes(bytes, offset)));
